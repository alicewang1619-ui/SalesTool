...
class LeadDetailOut(LeadOut):
    ...
    score_dimensions: list[ScoreDimensionOut] = Field(default_factory=list)
    score_total: int = 0
    score_max: int = 5
...
class NurtureTaskCreateRequest(BaseModel):
...
class BulkEmailFiltersIn(BaseModel):
    ...
    source_query: str | None = Field(default=None, max_length=120)
    feedback_status: str | None = Field(default=None, max_length=80)


class BulkEmailPreviewOut(BaseModel):
    filters: BulkEmailFiltersIn
    target_count: int
    recipients_preview: list["CustomerListItem"]
...
class SalesUserDeleteOut(BaseModel):
    deleted: bool
    user_id: int
    affected_leads: int
