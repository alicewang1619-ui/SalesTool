...
import { Check, Filter, Send } from "lucide-react";
...
export function CustomersPage() {
  ...
  async function load(nextFilters: CustomerFilters = filters) {
    setLoading(true);
    try {
      const result = await fetchCustomers(nextFilters);
      setData(result);
      ...
      <div className="page-heading">
        <div>
          ...
          <Typography.Paragraph className="muted">
            按高意向、有效跟进、资料库、已转代理商、无效、撤单/流失等状态沉淀客户。
          </Typography.Paragraph>
        </div>
        <Space wrap>
          <Button icon={<Filter size={16} />} onClick={() => form.submit()}>筛选</Button>
          <Button type="primary" icon={<Check size={16} />} onClick={() => message.success("已确认当前客户池视图")}>确认</Button>
        </Space>
      </div>

      <Row gutter={[16, 16]} className="metric-row">
        <Col xs={24} md={6}><Card><Statistic title="客户总数" value={data?.metrics.total_customers ?? 0} /></Card></Col>
            ...
            {summaryCustomer ? (
              <>
                ...
                <div className="timeline-list">
                  <div className="timeline-item"><strong>{summaryCustomer.background_summary}</strong><span>来源于客户背景调查与销售反馈</span></div>
                  <div className="timeline-item"><strong>客户状态：{summaryCustomer.tier}</strong><span>负责人：{summaryCustomer.owner_name}</span></div>
                  <div className="timeline-item"><strong>可进入客户详情继续修订背景调查</strong><span>{summaryCustomer.detail_path}</span></div>
                </div>
              </>
            ) : (
        ...
        <Col xs={24} lg={12}>
          <Card title="再营销草稿">
            <Input.TextArea
              ...
              value={
                summaryCustomer
                  ? `Hi ${summaryCustomer.name}, based on your ${summaryCustomer.product} interest, we prepared a short comparison for your team...`
                  : "暂无客户可生成草稿"
              }
              readOnly
            />
            <Button type="primary" icon={<Send size={16} />} style={{ marginTop: 16 }}>
              人工确认发送
            </Button>
          </Card>
        </Col>
      ...
      <Card className="table-card">
        <Table<CustomerListItem>
          ...
          locale={{
            emptyText: (
              <Empty
                description={data?.empty_state?.title ?? "暂无客户"}
          ...
          columns={[
            ...
            { title: "反馈", dataIndex: "owner_name", width: 150 },
            {
              ...
              fixed: "right",
              width: 130,
              render: (_, record) => <Link to={record.detail_path}><Button>查看详情</Button></Link>
            }
          ]}
          scroll={{ x: 1040 }}
        />
