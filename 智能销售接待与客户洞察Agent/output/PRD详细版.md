...
- 再营销草稿“重新生成”必须由后端大模型生成链路完成，结合邮件写手风格、技能、人工提示词、客户上下文和参考附件生成英文邮件模板参考；不允许用中文字段直接拼接英文正文。
- `draft_content` 面向后期人工调整，只返回英文邮件正文，不返回中文提示词、中文写手技能、模型内部说明或系统状态文案。
- 模型未配置或调用失败时，后端可用同一上下文生成英文兜底模板，但仍需记录模型供应商、模型版本和上下文快照，并继续坚持人工确认后发送。
## 2026-07-02 邮件写手角色展示补充
- 邮件写手角色下拉不得把风格、技能或标签直接拼在角色名后；下拉选项和选中态只展示英文角色名。
- 角色目标、能力方向、相关技能、背景定义和角色标签通过悬浮或点击角色名时的提示框展示，避免过长文本影响扫描。
- 设置页邮件写手卡片顶部只保留“新增写手”和“保存写手配置”；当前选中角色的“编辑”“删除”放到右侧详情卡片底部。

## 2026-07-03 营销技能菜单补充
- 邮件写手从“AI 与模型”中迁移到独立的“营销技能”配置菜单；用户从左侧“营销技能”进入时应直接看到邮件写手配置页，而不是先看到“配置中心”标题、说明、顶部配置 Tab 或其他配置项。
- “营销技能”第一版聚焦邮件写手技能配置：按不同英文写手维护角色目标、能力与技能方向、相关技能、背景定义、角色标签和后台执行提示词。
- 邮件写手配置页采用“左侧写手列表 + 右侧写手详情”的业务工具布局：左侧负责默认写手和当前写手选择，右侧展示当前写手的目标、能力、背景、执行提示词、技能标签，并在详情底部提供编辑和删除。
- “AI 与模型”只保留模型和知识入口；邮件写手数据仍复用现有 AI 配置接口字段保存，后续如需要再拆分独立接口。

## 2026-07-02 大模型连接配置操作补充
- 大模型连接配置顶部操作只保留“添加大模型”和“保存模型库”，用于新增模型和保存整体配置。
- 当前选中模型的“编辑”“删除”必须放在右侧模型详情卡片底部，避免和新增/保存混在一起导致用户误以为作用于整个模型库。

## 2026-07-02 全站信息型蓝框清理补充
- 工作台、线索池、客户详情、报表、个人页、设置页等业务页面不得用蓝色 `info` Alert 铺开展示筛选范围、字段说明、用途说明或“已选中”状态。
- 这类辅助信息应改为普通灰色文字、卡片内摘要、Tooltip 或控件旁轻量提示；页面主流程优先展示筛选、表单、指标、列表和可执行动作。
- 需要用户处理的错误、警告、成功反馈和导入/导出结果状态可继续使用对应状态提示，但不得伪装成大块蓝色说明框。

## 2026-07-02 全站阶段标识清理补充
- 运行态后台页面的标题区域不得展示“阶段 1”“阶段 2”“MVP”等产品开发阶段标签；这些信息属于内部规划，不应暴露给业务使用者。
- 页面顶部只保留业务标题、业务说明和必要动作，例如“工作台首页”“再营销与邮件活动”“配置中心”等。
- 文档、文件名或内部路线图仍可保留阶段划分，用于项目管理；但前端 `frontend/src/pages` 渲染出的业务页面不得出现这类阶段标识。
## 2026-07-03 最新需求口径：营销技能与知识库左侧独立入口
- “营销技能”从配置中心移出，成为左侧主导航一级入口；第一版只承载邮件写手配置，强调邮件写手是一套可复用的营销 Prompt 技能。
- “知识库”从配置中心和“AI 与模型”中移出，成为左侧主导航一级入口；默认维护产品、竞品、市场和自定义知识库板块。
- 配置中心不再展示“营销技能”Tab，也不再在“AI 与模型”中展示知识库入口卡；AI 与模型只处理模型场景绑定和大模型连接。
- 写邮件、再营销和群发邮件生成时，系统按场景自动调用启用知识库作为大模型上下文，辅助生成邮件，不要求用户回设置中心查找资料。
- 知识库页面不再展示顶部统计卡或“只看启用”等总览按钮；页面重点改为知识分类、知识内容、关键词标签和 PDF/Word/TXT 资料上传解析。
- 知识库默认分类扩展为产品知识、竞品知识、市场知识、商务成果；后续仍允许新增自定义板块。

## 2026-07-03 最新需求口径：主动挖潜客
- 左侧主导航新增“挖潜客”，面向管理员和运营，用于在已有线索池之外主动规划拓客、生成候选潜客并人工确认入库；销售角色默认不可见、不可访问接口。
- 挖潜客流程为：填写选填品牌、产品侧重点、目标区域、平台式客户画像、来源渠道和本次挖取数量 → 系统生成 AI 拓客方案与渠道节奏 → 按数量上限输出候选客户 → 用户查看来源链接和匹配理由 → 人工确认后写入正式线索池。
- 客户画像不能只是一段自由文本，必须像 Facebook / LinkedIn 受众画像一样结构化配置：行业/机构类型、岗位或采购角色、公司规模或渠道类型、应用场景、意图关键词、排除条件和来源渠道。系统必须基于这些字段生成搜索关键词、渠道链接和候选客户。
- 品牌不是必填项；来源渠道必须支持从 Google、LinkedIn、Google Maps、Facebook、人工来源等预设项中多选，也支持运营直接新增自定义来源标签，并在候选客户与入库线索中保留这些来源。
- 每次挖潜客必须显式设置“本次挖取数量”，默认 20 个，后端允许 1–100 个并严格按该数量生成候选，避免无限挖取或一次生成过多数据。
- 候选客户必须记录来源渠道、来源说明和来源链接；确认入库后，这些来源追溯信息同步写入线索记录和客户详情，方便后续判断客户从哪里挖来。
- 支持 Google、LinkedIn、Google Maps 等渠道作为拓客入口；LinkedIn/Facebook 仍遵守既有合规边界，只提供授权/公开搜索入口和来源链接，不做未授权抓取或个人后台自动采集。
- 候选客户未确认前不得直接进入线索池或客户池；确认入库要事务化，避免重复入库，并保留审计记录。
