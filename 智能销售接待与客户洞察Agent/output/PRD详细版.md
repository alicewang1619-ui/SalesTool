...
- 挖潜客流程为：填写品牌/产品侧重点、目标区域、平台式客户画像和渠道 → 系统生成 AI 拓客方案与渠道节奏 → 输出候选客户 → 用户查看来源链接和匹配理由 → 人工确认后写入正式线索池。
- 客户画像不能只是一段自由文本，必须像 Facebook / LinkedIn 受众画像一样结构化配置：行业/机构类型、岗位或采购角色、公司规模或渠道类型、应用场景、意图关键词、排除条件和拓客渠道。系统必须基于这些字段生成搜索关键词、渠道链接和候选客户。
