...
**功能描述**: 系统接收官网聊天、官网后台、邮箱和展会名片导入的询盘，统一生成 Inquiry。每条线索必须记录来源渠道、接收时间、原始内容、客户联系方式和当前处理状态。重复线索不能直接覆盖，需要提示可能重复并允许人工合并。该功能支撑 US-1。

| 元素 | 类型 | 默认态 | 操作后 | 禁用条件 |
|---|---|---|---|---|
| 渠道筛选 | 筛选器 | 全部渠道 | 只显示选中渠道 | 无线索时不可用 |
...
**功能描述**: 系统生成日报、月报、季报、年报所需的非金额型经营分析。指标包括询盘总量、有效数量、国家分布、渠道质量、代理商/医生占比、产品分类、销售反馈、未及时反馈、撤单情况、再营销数据、客户池变化。明确不统计成交金额和报价金额。该功能支撑 US-7。
