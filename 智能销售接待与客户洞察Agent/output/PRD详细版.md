...
| 阶段 | 验证目标 | 功能模块 | 交付物 |
|---|---|---|---|
| 阶段一 MVP | 跑通“询盘进入、AI 问清、销售收到、销售反馈、管理者看报表”的最短闭环 | FR-1、FR-2、FR-3、FR-4、FR-5、FR-6、FR-7、FR-8、FR-9 | 可用的统一线索池、AI 接待、客户画像评分、国家分发、轻量反馈、基础报表 |
| 阶段二 增强 | 提高渠道覆盖和客户经营能力 | FR-10、FR-11、FR-12 | 再营销草稿队列、客户态势信号记录、更多渠道接入 |
| 阶段三 扩展 | 在数据闭环稳定后连接更复杂系统 | 后续扩展模块 | CRM 深度集成、企业微信深度集成、完整客户态势监督 |

> MVP 完成定义：完成 FR-1 至 FR-9 后，第一版 MVP 即视为完成。MVP 必须让官网聊天、官网后台、邮箱、展会名片导入的询盘进入统一线索池，并完成 AI 接待引导、客户画像评分、国家分发、销售轻反馈和基础报表。
...
| 管理员 | 维护系统规则 | 维护国家/区域/销售映射、产品知识库、渠道导入 | 规则失效后无人发现 |
...
- **US-10**: 作为营销负责人，我希望 AI 只生成再营销草稿，由人工确认后发送，避免误发。（由 FR-10 实现）
...
**功能描述**: 系统接收网站、邮箱、社媒、线下展会、其他五类来源的询盘，统一生成 Inquiry。每条线索必须记录一级来源途径、具体来源标签、接收时间、原始内容、客户联系方式和当前处理状态；官网聊天、官网后台归入“网站”，Facebook、领英归入“社媒”，展会名片导入归入“线下展会”。Facebook、领英可作为人工导入或授权归档的来源标签进入筛选和列表展示，但不代表 MVP 自动抓取或接入个人后台。重复线索不能直接覆盖，需要提示可能重复并允许人工合并。该功能支撑 US-1。

| 元素 | 类型 | 默认态 | 操作后 | 禁用条件 |
|---|---|---|---|---|
| 来源筛选 | 筛选器 | 全部来源 | 按网站、邮箱、社媒、线下展会、其他五类一级来源筛选 | 无线索时不可用 |
| 来源列 | 表格列 | 展示一级来源 + 具体来源 | 一级来源固定为网站、邮箱、社媒、线下展会、其他；具体来源可为官网聊天、官网后台、Facebook、领英、邮箱、线下展会导入等 | 无来源时标记其他/未知 |
...
**位置**: 线索详情页、销售线索卡片、报表明细。

**功能描述**: 系统根据对话、客户回答、公开资料和销售反馈生成客户背景摘要，并按身份明确、应用明确、需求明确三道硬门槛判断有效性。评分结果必须展示理由，不能只展示等级。该功能支撑 US-4。
...
**功能描述**: 系统把客户按状态沉淀到客户池，包括高意向客户、有效客户持续跟进、已报价未回复、暂无明确需求、已转代理商跟进、无效客户、撤单/流失客户、已成交客户。客户池变化进入报表。该功能支撑 US-8。
...
**功能描述**: 系统根据客户池状态生成再营销建议、邮件草稿、触达理由和推荐时间。第一版必须人工确认后发送，不做未经确认的自动群发。该功能支撑 US-10。
...
**功能描述**: 管理员维护国家/区域/销售映射、产品知识库、AI 接待 playbook、反馈提醒规则、渠道导入配置和权限。配置变更需要记录操作人和时间。该功能支撑所有核心流程。
...
| 管理员配置完整率 | 待建立 | 国家映射和知识库有维护入口 | MVP 上线后 1 个月 | 来源：方案假设，待验证 |
...
| AC-14 | 再营销草稿 | AI 生成草稿但需人工确认 | 主流程 | FR-10 |
...
- 用户要求客户来源一级分类固定为网站、邮箱、社媒、线下展会、其他；Facebook/领英作为“社媒”下的具体来源标签，不改变“不接未授权 API / 不抓取个人后台”的边界。
...
| 线索来源筛选与展示 | `source_group` 记录一级来源：网站、邮箱、社媒、线下展会、其他；`source_channel` 记录具体来源：官网聊天、官网后台、Facebook、领英、邮箱、线下展会导入等；Facebook/领英只作为授权或手工来源标签 | 前端列表筛选、后端 Inquiry 查询条件和渠道字典配置 | Inquiry 查询结果 |
