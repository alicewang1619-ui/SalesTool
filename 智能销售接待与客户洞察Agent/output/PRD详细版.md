...
**功能描述**: 系统接收官网聊天、官网后台、邮箱和展会名片导入的询盘，统一生成 Inquiry。每条线索必须记录来源渠道、来源分组、接收时间、原始内容、客户联系方式和当前处理状态；Facebook、领英可作为人工导入或授权归档的来源标签进入筛选和列表展示，但不代表 MVP 自动抓取或接入个人后台。重复线索不能直接覆盖，需要提示可能重复并允许人工合并。该功能支撑 US-1。

| 元素 | 类型 | 默认态 | 操作后 | 禁用条件 |
|---|---|---|---|---|
| 来源筛选 | 筛选器 | 全部来源 | 只显示选中来源；官网为官网聊天/官网后台聚合，社媒为 Facebook/领英聚合 | 无线索时不可用 |
| 来源列 | 表格列 | 展示具体来源 | 支持官网聊天、官网后台、Facebook、领英、邮箱、线下展会导入 | 无来源时标记未知渠道 |
...
- 用户要求线索池来源筛选新增 Facebook、领英、邮箱、线下展会导入；本版将 Facebook/领英作为手工导入或授权归档的来源标签，不改变“不接未授权 API / 不抓取个人后台”的边界。
...
| 线索来源筛选与展示 | `source_channel` 记录具体来源，`source_group` 记录官网/社媒/邮箱/线下展会等聚合来源；Facebook/领英只作为授权或手工来源标签 | 前端列表筛选、后端 Inquiry 查询条件和渠道字典配置 | Inquiry 查询结果 |
