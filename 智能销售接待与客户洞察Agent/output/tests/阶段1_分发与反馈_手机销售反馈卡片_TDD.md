...
## 2026-07-02 追加验收
- `GET /api/feedback-links/:token` 返回状态选项必须为：无效、跟进中、已报价、已签单、已付款、价格流失、撤单；未分配只作为未分配销售时的系统状态，不出现在销售反馈按钮里。
- 提交任一新状态后，线索、客户详情、报表和反馈历史使用同一中文状态值。
- AI 判断理由必须包含 5 个评分维度，且字段名为中文。
