...
## 2026-07-03 最新验收：知识库左侧独立入口
- PAGE-KB-NAV-01：管理员/运营从左侧主导航点击“知识库”进入 `/admin/knowledge-base`，左侧菜单高亮“知识库”。
- PAGE-KB-NAV-02：旧 `/admin/settings/product-knowledge` 仅作为兼容入口，打开时仍显示同一个知识库页面并高亮左侧“知识库”。
- PAGE-KB-NAV-03：知识库支持产品、竞品、市场和自定义板块维护；新增、编辑、启用/停用和删除都要保留审计与安全边界。
- PAGE-KB-NAV-04：写邮件、再营销重新生成和群发邮件生成读取启用知识库作为大模型数据上下文，不依赖设置中心入口卡。

## 2026-07-03 最新验收：知识库内容、标签与上传
- PAGE-KB-CONTENT-01：知识库页面不展示顶部统计卡、当前版本卡、“只看启用”按钮或统计型总览文案。
- PAGE-KB-CONTENT-02：管理员可保存产品知识、竞品知识、市场知识、商务成果或自定义板块条目，每条条目展示关键词标签。
- PAGE-KB-CONTENT-03：保存标签后，列表响应包含 `tags`，AI 上下文 Prompt 包含 `Tags:` 行，避免标签只停留在前端。
- PAGE-KB-CONTENT-04：上传 Word/PDF/TXT 资料后，后端返回可读正文和建议标签，前端填入知识内容，保存前仍允许人工修改。
