...
## 前端进度返修 TDD 补充（06-30-2026）

- 页面上传文件后必须调用 `fetchImportJob(task_id)` 查询任务状态；不能只用创建任务响应渲染最终结果。
- 页面文案必须区分 queued 创建态与 completed 完成态，避免在后台任务未完成时提示“已完成”。

## 前端进度风险返修补充（06-30-2026）

- 前端轮询应覆盖较慢导入任务，当前约 30 秒窗口内持续查询任务状态。
- queued/processing/completed 的页面说明必须按状态展示，不能在 queued 阶段写完成态文案。
## 2026-07-01 二次统一返修 TDD
- `GET /api/import-template` 必须返回可下载 CSV 模板，响应头包含附件文件名，模板包含必填字段和示例行。
- 上传 CSV/XLSX 后页面显示文件名、格式校验、任务状态、成功/失败明细和按国家销售映射的自动分配结果。
- 非 CSV/XLSX 文件必须被拒绝，并提示“请上传 CSV 或 Excel 表格”。
